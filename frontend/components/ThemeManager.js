...
export default function ThemeManager({ weddingId, wedding }) {
  ...
  const loadLayoutPhotos = async () => {
    try {
      ...
      // Process photo URLs to handle both Telegram and local URLs with fallback
      Object.keys(photos).forEach(slotName => {
        const slotPhotos = photos[slotName];
        if (Array.isArray(slotPhotos)) {
          ...
          slotPhotos.forEach((photo, index) => {
            if (photo && photo.url) {
              ...
              // Try local proxy first, then fallback to direct Telegram URL
              if (photo.url.includes('api.telegram.org')) {
                // Convert Telegram URL to local proxy URL
                const telegramFileId = photo.url.split('/photos/')[1];
                if (telegramFileId) {
                  const apiBaseUrl = getApiBaseUrl();
                  // Extract file_id from Telegram URL and store it for proper resolution
                  const fileId = telegramFileId.split('.')[0]; // Remove extension if present
                  photo.url = `${apiBaseUrl}${CONFIG.MEDIA.PROXY_ENDPOINT}${fileId}`;
                  photo.fallbackUrl = `${apiBaseUrl}${CONFIG.MEDIA.PROXY_ENDPOINT}${fileId}`;
                  photo.file_id = fileId; // Store clean file_id for reference
                  console.log('Converted Telegram URL to local proxy with fallback:', { 
                    proxy: photo.url, 
                ...
                console.log('Keeping original external URL:', photo.url);
                photo.fallbackUrl = null; // No fallback needed
              } else {
                // Transform local relative URLs to absolute API URLs
                const apiBaseUrl = getApiBaseUrl();
                const normalizedUrl = photo.url.startsWith('/api/') ? photo.url : `/api${photo.url}`;
                photo.url = `${apiBaseUrl}${normalizedUrl}`;
                console.log('Transformed local photo URL:', photo.url);
          ...
          // Handle single photo
          const originalUrl = slotPhotos.url;
          
          if (slotPhotos.url.includes('api.telegram.org')) {
            // Convert Telegram URL to local proxy URL
            const telegramFileId = slotPhotos.url.split('/photos/')[1];
            if (telegramFileId) {
              const apiBaseUrl = getApiBaseUrl();
              // Extract file_id from Telegram URL and store it for proper resolution
              const fileId = telegramFileId.split('.')[0]; // Remove extension if present
              slotPhotos.url = `${apiBaseUrl}${CONFIG.MEDIA.PROXY_ENDPOINT}${fileId}`;
              slotPhotos.fallbackUrl = `${apiBaseUrl}${CONFIG.MEDIA.PROXY_ENDPOINT}${fileId}`;
              slotPhotos.file_id = fileId; // Store clean file_id for reference
              console.log('Converted Telegram URL to local proxy with fallback:', { 
                proxy: slotPhotos.url, 
                fallback: slotPhotos.fallbackUrl,
                file_id: fileId
              });
            }
          } else if (slotPhotos.url.startsWith('http://') || slotPhotos.url.startsWith('https://')) {
            console.log('Keeping original external URL:', slotPhotos.url);
            slotPhotos.fallbackUrl = null;
          } else {
            // Transform local relative URLs to absolute API URLs
            const apiBaseUrl = getApiBaseUrl();
            const normalizedUrl = slotPhotos.url.startsWith('/api/') ? slotPhotos.url : `/api${slotPhotos.url}`;
            slotPhotos.url = `${apiBaseUrl}${normalizedUrl}`;
            console.log('Transformed local photo URL:', slotPhotos.url);
            slotPhotos.fallbackUrl = null;
          }
          
          // Add retry mechanism flag
          slotPhotos.retryCount = 0;
          slotPhotos.maxRetries = CONFIG.IMAGE.MAX_RETRIES;
        }
      });
  ...
  const handleUpdateBackgrounds = async (updates) => {
    try {
      ...
      toast.success('Backgrounds updated!');
      loadBackgrounds();
    } catch (error) {
      console.error('Error updating backgrounds:', error);
      toast.error('Failed to update backgrounds');
    } finally {
      setLoading(false);
    }
  };
