...
export default function ThemeManager({ weddingId, wedding }) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Typography & Colors Card - REMOVED as per user request */}
          ...
          {loadingAssets ? (
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                {/* Preview Section - Debug */}
                <div className="mt-2">
                  {backgrounds.stream_page_background_id && !backgrounds.stream_page_background_url && (
                    <div className="p-2 bg-amber-50 border border-amber-200 rounded text-xs text-amber-700">
                      Background selected but URL not loaded. Check console for details.
                    </div>
                  )}
                  {backgrounds.stream_page_background_url && (
                    <div className="p-2 bg-gray-50 rounded-lg border">
                      ...
                      <img 
                        ...
                        className="w-full h-32 object-cover rounded"
                        onError={(e) => {
                          console.error('[BACKGROUND_IMAGE_ERROR] Failed to load:', backgrounds.stream_page_background_url);
                          e.target.style.display = 'none';
                        }}
                      />
                      <p className="text-[10px] text-gray-400 mt-1 break-all">
                        URL: {backgrounds.stream_page_background_url}
                      </p>
                    </div>
                  )}
                </div>
