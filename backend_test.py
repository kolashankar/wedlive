...
# Get backend URL from environment - use port 8001 as mentioned in review request
...
class WedLiveAPITester:
    def __init__(self):
        ...
        self.results = {
            ...
            "borders": {"passed": 0, "failed": 0, "errors": []},
            "backgrounds": {"passed": 0, "failed": 0, "errors": []},
            "precious_styles": {"passed": 0, "failed": 0, "errors": []},
            "sections": {"passed": 0, "failed": 0, "errors": []},
            "studios": {"passed": 0, "failed": 0, "errors": []}
        }
    ...
    # ==================== WEDLIVE DYNAMIC BORDER + MASK SYSTEM TESTS ====================
    ...
    async def test_admin_login(self):
        ...
        if success and status == 200:
            if "access_token" in response and "user" in response:
                ...
                self.log_result("authentication", "Admin Login", False, "Missing access_token or user in response")
        else:
            self.log_result("authentication", "Admin Login", False, f"Status {status}: {response}")
        
        return False
    
    async def test_creator_login(self):
        """Test creator login with provided credentials"""
        creator_credentials = {
            "email": "kolashankar423@gmail.com",
            "password": "Shankar@113"
        }
        
        success, response, status = await self.make_request("POST", "/auth/login", creator_credentials)
        
        if success and status == 200:
            if "access_token" in response and "user" in response:
                # Store creator token separately
                self.creator_token = response["access_token"]
                self.log_result("authentication", "Creator Login", True)
                return True
            else:
                self.log_result("authentication", "Creator Login", False, "Missing access_token or user in response")
        else:
            self.log_result("authentication", "Creator Login", False, f"Status {status}: {response}")
    ...
    async def test_auth_me_endpoint(self):
        ...
        if not self.auth_token:
            self.log_result("authentication", "Auth Me Endpoint", False, "No auth token")
            return False
        
        success, response, status = await self.make_request("GET", "/auth/me")
        
        if success and status == 200:
            if "id" in response and "email" in response and "role" in response:
                self.log_result("authentication", "Auth Me Endpoint", True)
                return True
            else:
                self.log_result("authentication", "Auth Me Endpoint", False, "Missing required fields in response")
        else:
            self.log_result("authentication", "Auth Me Endpoint", False, f"Status {status}: {response}")
        
        return False
    
    # ==================== BORDER MANAGEMENT API TESTS ====================
    
    async def test_get_borders_public(self):
        """Test GET /api/borders (public - browse borders)"""
        success, response, status = await self.make_request("GET", "/borders")
        
        if success and status == 200:
            if isinstance(response, list):
                self.log_result("borders", "Get Borders (Public)", True)
                print(f"   ✅ Found {len(response)} borders")
                return True
            else:
                self.log_result("borders", "Get Borders (Public)", False, "Response is not a list")
        else:
            self.log_result("borders", "Get Borders (Public)", False, f"Status {status}: {response}")
        
        return False
    
    async def test_get_border_by_id(self):
        """Test GET /api/borders/{id} (get border details)"""
        # First get list of borders to get an ID
        success, borders, status = await self.make_request("GET", "/borders")
        
        if not success or not borders or len(borders) == 0:
            self.log_result("borders", "Get Border By ID", False, "No borders available to test")
            return False
        
        border_id = borders[0]["id"]
        success, response, status = await self.make_request("GET", f"/borders/{border_id}")
        
        if success and status == 200:
            if "id" in response and "name" in response and "cdn_url" in response:
                self.log_result("borders", "Get Border By ID", True)
                return True
            else:
                self.log_result("borders", "Get Border By ID", False, "Missing required fields in response")
        else:
            self.log_result("borders", "Get Border By ID", False, f"Status {status}: {response}")
        
        return False
    
    async def test_admin_borders_list(self):
        """Test GET /api/admin/borders (admin - list all borders)"""
        if not hasattr(self, 'admin_token') or not self.admin_token:
            self.log_result("borders", "Admin Borders List", False, "No admin token")
            return False
        ...
        success, response, status = await self.make_request("GET", "/admin/borders")
        ...
        if success and status == 200:
            if isinstance(response, list):
                self.log_result("borders", "Admin Borders List", True)
                print(f"   ✅ Admin can see {len(response)} borders")
                return True
            else:
                self.log_result("borders", "Admin Borders List", False, "Response is not a list")
        else:
            self.log_result("borders", "Admin Borders List", False, f"Status {status}: {response}")
        
        return False
    
    # ==================== BACKGROUND IMAGES API TESTS ====================
    
    async def test_get_backgrounds_public(self):
        """Test GET /api/theme-assets/backgrounds (public)"""
        success, response, status = await self.make_request("GET", "/theme-assets/backgrounds")
        
        if success and status == 200:
            if isinstance(response, list):
                self.log_result("backgrounds", "Get Backgrounds (Public)", True)
                print(f"   ✅ Found {len(response)} background images")
                return True
            else:
                self.log_result("backgrounds", "Get Backgrounds (Public)", False, "Response is not a list")
        else:
            self.log_result("backgrounds", "Get Backgrounds (Public)", False, f"Status {status}: {response}")
        
        return False
    
    async def test_admin_backgrounds_list(self):
        """Test GET /api/admin/theme-assets/backgrounds (admin)"""
        if not hasattr(self, 'admin_token') or not self.admin_token:
            self.log_result("backgrounds", "Admin Backgrounds List", False, "No admin token")
            return False
        ...
        success, response, status = await self.make_request("GET", "/admin/theme-assets/backgrounds")
        ...
        if success and status == 200:
            if isinstance(response, list):
                self.log_result("backgrounds", "Admin Backgrounds List", True)
                print(f"   ✅ Admin can see {len(response)} background images")
                return True
            else:
                self.log_result("backgrounds", "Admin Backgrounds List", False, "Response is not a list")
        else:
            self.log_result("backgrounds", "Admin Backgrounds List", False, f"Status {status}: {response}")
        
        return False
    
    # ==================== PRECIOUS MOMENTS STYLES API TESTS ====================
    
    async def test_get_precious_styles_public(self):
        """Test GET /api/theme-assets/precious-styles (public)"""
        success, response, status = await self.make_request("GET", "/theme-assets/precious-styles")
        
        if success and status == 200:
            if isinstance(response, list):
                self.log_result("precious_styles", "Get Precious Styles (Public)", True)
                print(f"   ✅ Found {len(response)} precious moment styles")
                return True
            else:
                self.log_result("precious_styles", "Get Precious Styles (Public)", False, "Response is not a list")
        else:
            self.log_result("precious_styles", "Get Precious Styles (Public)", False, f"Status {status}: {response}")
        
        return False
    
    async def test_get_precious_style_by_id(self):
        """Test GET /api/theme-assets/precious-styles/{style_id} (public)"""
        # First get list of styles to get an ID
        success, styles, status = await self.make_request("GET", "/theme-assets/precious-styles")
        
        if not success or not styles or len(styles) == 0:
            self.log_result("precious_styles", "Get Precious Style By ID", False, "No styles available to test")
            return False
        
        style_id = styles[0]["id"]
        success, response, status = await self.make_request("GET", f"/theme-assets/precious-styles/{style_id}")
        
        if success and status == 200:
            if "id" in response and "name" in response and "layout_type" in response:
                self.log_result("precious_styles", "Get Precious Style By ID", True)
                return True
            else:
                self.log_result("precious_styles", "Get Precious Style By ID", False, "Missing required fields in response")
        else:
            self.log_result("precious_styles", "Get Precious Style By ID", False, f"Status {status}: {response}")
        
        return False
    
    async def test_admin_precious_styles_list(self):
        """Test GET /api/admin/theme-assets/precious-styles (admin)"""
        if not hasattr(self, 'admin_token') or not self.admin_token:
            self.log_result("precious_styles", "Admin Precious Styles List", False, "No admin token")
            return False
        ...
        success, response, status = await self.make_request("GET", "/admin/theme-assets/precious-styles")
        ...
        if success and status == 200:
            if isinstance(response, list):
                self.log_result("precious_styles", "Admin Precious Styles List", True)
                print(f"   ✅ Admin can see {len(response)} precious moment styles")
                return True
            else:
                self.log_result("precious_styles", "Admin Precious Styles List", False, "Response is not a list")
        else:
            self.log_result("precious_styles", "Admin Precious Styles List", False, f"Status {status}: {response}")
        
        return False
    
    # ==================== SECTION CONFIGURATION API TESTS ====================
    
    async def test_get_wedding_sections(self):
        """Test GET /api/weddings/{wedding_id}/sections (get section config)"""
        if not self.test_wedding_id:
            self.log_result("sections", "Get Wedding Sections", False, "No test wedding created")
            return False
        
        success, response, status = await self.make_request("GET", f"/weddings/{self.test_wedding_id}/sections")
        
        if success and status == 200:
            # Should return section configuration object
            if isinstance(response, dict):
                self.log_result("sections", "Get Wedding Sections", True)
                return True
            else:
                self.log_result("sections", "Get Wedding Sections", False, "Response is not a dict")
        else:
            self.log_result("sections", "Get Wedding Sections", False, f"Status {status}: {response}")
        
        return False
    
    async def test_update_cover_section(self):
        """Test PUT /api/weddings/{wedding_id}/sections/cover (creator - update cover section)"""
        if not self.test_wedding_id or not self.auth_token:
            self.log_result("sections", "Update Cover Section", False, "Missing wedding ID or auth token")
            return False
        
        update_data = {
            "mode": "couple",
            "couple_border_id": None,
            "selected_border_id": None
        }
        
        success, response, status = await self.make_request("PUT", f"/weddings/{self.test_wedding_id}/sections/cover", update_data)
        
        if success and status == 200:
            if "success" in response and response["success"]:
                self.log_result("sections", "Update Cover Section", True)
                return True
            else:
                self.log_result("sections", "Update Cover Section", False, "Success field not true in response")
        else:
            self.log_result("sections", "Update Cover Section", False, f"Status {status}: {response}")
        
        return False
    
    async def test_update_studio_section(self):
        """Test PUT /api/weddings/{wedding_id}/sections/studio (creator - update studio section)"""
        if not self.test_wedding_id or not self.auth_token:
            self.log_result("sections", "Update Studio Section", False, "Missing wedding ID or auth token")
            return False
        
        update_data = {
            "studio_id": None,
            "studio_border_id": None
        }
        
        success, response, status = await self.make_request("PUT", f"/weddings/{self.test_wedding_id}/sections/studio", update_data)
        
        if success and status == 200:
            if "success" in response and response["success"]:
                self.log_result("sections", "Update Studio Section", True)
                return True
            else:
                self.log_result("sections", "Update Studio Section", False, "Success field not true in response")
        else:
            self.log_result("sections", "Update Studio Section", False, f"Status {status}: {response}")
        
        return False
    
    # ==================== STUDIO MANAGEMENT API TESTS ====================
    
    async def test_create_studio(self):
        """Test POST /api/studios (creator - create studio)"""
        if not self.auth_token:
            self.log_result("studios", "Create Studio", False, "No auth token")
            return False
        ...
        success, response, status = await self.make_request("POST", "/studios", studio_data)
        
        if success and status == 200:
            if "id" in response and "name" in response:
                self.test_studio_id = response["id"]
                self.log_result("studios", "Create Studio", True)
                return True
            else:
                self.log_result("studios", "Create Studio", False, "Missing required fields in response")
        else:
            self.log_result("studios", "Create Studio", False, f"Status {status}: {response}")
        
        return False
    
    async def test_list_studios(self):
        """Test GET /api/studios (creator - list studios)"""
        if not self.auth_token:
            self.log_result("studios", "List Studios", False, "No auth token")
            return False
        
        success, response, status = await self.make_request("GET", "/studios")
        
        if success and status == 200:
            if isinstance(response, list):
                self.log_result("studios", "List Studios", True)
                print(f"   ✅ Found {len(response)} studios")
                return True
            else:
                self.log_result("studios", "List Studios", False, "Response is not a list")
        else:
            self.log_result("studios", "List Studios", False, f"Status {status}: {response}")
        
        return False
    
    async def test_get_studio(self):
        """Test GET /api/studios/{id} (creator - get studio)"""
        if not hasattr(self, 'test_studio_id') or not self.test_studio_id:
            self.log_result("studios", "Get Studio", False, "No test studio created")
            return False
        
        success, response, status = await self.make_request("GET", f"/studios/{self.test_studio_id}")
        
        if success and status == 200:
            if "id" in response and "name" in response:
                self.log_result("studios", "Get Studio", True)
                return True
            else:
                self.log_result("studios", "Get Studio", False, "Missing required fields in response")
        else:
            self.log_result("studios", "Get Studio", False, f"Status {status}: {response}")
        
        return False
    
    async def test_update_studio(self):
        """Test PUT /api/studios/{id} (creator - update studio)"""
        if not hasattr(self, 'test_studio_id') or not self.test_studio_id:
            self.log_result("studios", "Update Studio", False, "No test studio created")
            return False
        
        update_data = {
            "name": "Updated Test Photography Studio",
            "phone": "+1987654321"
        }
        
        success, response, status = await self.make_request("PUT", f"/studios/{self.test_studio_id}", update_data)
        
        if success and status == 200:
            if "id" in response and response["name"] == "Updated Test Photography Studio":
                self.log_result("studios", "Update Studio", True)
                return True
            else:
                self.log_result("studios", "Update Studio", False, "Studio not updated correctly")
        else:
            self.log_result("studios", "Update Studio", False, f"Status {status}: {response}")
        
        return False
    
    async def test_delete_studio(self):
        """Test DELETE /api/studios/{id} (creator - delete studio)"""
        if not hasattr(self, 'test_studio_id') or not self.test_studio_id:
            self.log_result("studios", "Delete Studio", False, "No test studio created")
            return False
        
        success, response, status = await self.make_request("DELETE", f"/studios/{self.test_studio_id}")
        
        if success and status == 200:
            if "success" in response and response["success"]:
                self.log_result("studios", "Delete Studio", True)
                return True
            else:
                self.log_result("studios", "Delete Studio", False, "Success field not true in response")
        else:
            self.log_result("studios", "Delete Studio", False, f"Status {status}: {response}")
        
        return False
    
    # ==================== WEDDING MANAGEMENT TESTS ====================
    
    async def test_create_wedding_endpoint(self):
        """Test POST /api/weddings (creator - create wedding)"""
        if not self.auth_token:
            self.log_result("weddings", "Create Wedding Endpoint", False, "No auth token")
            return False
        
        wedding_data = {
            "title": "Dynamic Border Test Wedding",
            "description": "Testing the new dynamic border system",
            "bride_name": "Sarah Johnson",
        ...
        if success and status == 201:
            if "id" in response and "title" in response:
                self.test_wedding_id_2 = response["id"]
                self.log_result("weddings", "Create Wedding Endpoint", True)
                return True
            else:
                self.log_result("weddings", "Create Wedding Endpoint", False, "Missing required fields in response")
        else:
            self.log_result("weddings", "Create Wedding Endpoint", False, f"Status {status}: {response}")
        
        return False
    
    async def test_get_wedding_details_endpoint(self):
        """Test GET /api/weddings/{id} (get wedding details)"""
        wedding_id = getattr(self, 'test_wedding_id_2', None) or self.test_wedding_id
        if not wedding_id:
            self.log_result("weddings", "Get Wedding Details Endpoint", False, "No test wedding created")
            return False
        
        success, response, status = await self.make_request("GET", f"/weddings/{wedding_id}")
        
        if success and status == 200:
            required_fields = ["id", "title", "bride_name", "groom_name", "status"]
            if all(field in response for field in required_fields):
                self.log_result("weddings", "Get Wedding Details Endpoint", True)
                return True
            else:
                missing_fields = [field for field in required_fields if field not in response]
                self.log_result("weddings", "Get Wedding Details Endpoint", False, f"Missing fields: {missing_fields}")
        else:
            self.log_result("weddings", "Get Wedding Details Endpoint", False, f"Status {status}: {response}")
