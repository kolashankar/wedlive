...
async def send_reaction(
    ...
    reaction_doc = {
        ...
        "user_id": current_user["id"] if current_user else None,
        "guest_name": current_user.get("full_name") if current_user else reaction.guest_name,
...
async def get_reactions(
    ...
    """Get recent reactions for a wedding (last 50)"""
    db = await get_database()
...
async def get_guest_book_entries(
    ...
    """Get guest book entries for a wedding (public access)"""
    db = await get_database()
