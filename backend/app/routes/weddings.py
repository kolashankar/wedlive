...
async def update_wedding_backgrounds(
    ...
    try:
        # Merge partial updates with existing backgrounds object
        current_theme_settings = wedding.get("theme_settings", {}) if isinstance(wedding.get("theme_settings"), dict) else {}
        current_theme_assets = current_theme_settings.get("theme_assets", {}) if isinstance(current_theme_settings.get("theme_assets"), dict) else {}
        current_backgrounds = current_theme_assets.get("backgrounds", {}) if isinstance(current_theme_assets.get("backgrounds"), dict) else {}
        ...
        result = await db.weddings.update_one(
            {"id": wedding_id},
            {"$set": {
                "theme_settings.theme_assets.backgrounds": merged_backgrounds,
                "updated_at": datetime.utcnow()
            }}
        )
        ...
        async def _resolve_bg(bg_id: str):
            if not bg_id:
                return None
            doc = await db.background_images.find_one({"id": bg_id})
        ...
        response = {
            ...
            "layout_page_background_url": await _resolve_bg(merged_backgrounds.get("layout_page_background_id")),
            "stream_page_background_url": await _resolve_bg(merged_backgrounds.get("stream_page_background_id")),
        }
...
async def get_wedding_backgrounds(
    ...
    theme_settings = wedding.get("theme_settings", {}) if isinstance(wedding.get("theme_settings"), dict) else {}
    theme_assets = theme_settings.get("theme_assets", {}) if isinstance(theme_settings.get("theme_assets"), dict) else {}
    backgrounds = theme_assets.get("backgrounds", {}) if isinstance(theme_assets.get("backgrounds"), dict) else {}
