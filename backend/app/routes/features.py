...
async def send_email_invitations(
    ...
    if wedding["creator_id"] != current_user["id"] and current_user["role"] != "admin":
...
async def create_camera_stream(
    ...
    # Create stream call for this camera
    try:
        stream_data = await create_stream_call(f"{wedding['title']} - {camera.camera_name}")
...
async def delete_photo_booth_photo(
    ...
    is_owner = current_user and photo.get("user_id") == current_user["id"]
    is_creator = current_user and wedding["creator_id"] == current_user["id"]
