#!/usr/bin/env python3
"""
Simple database cleanup to remove invalid file_XX references
"""
import asyncio
import sys
import os
...
os.environ['DATABASE_NAME'] = 'record_db'
...
async def cleanup_invalid_file_references():
    ...
    try:
        ...
        # Find all weddings with layout_photos
        weddings = await db.weddings.find({"layout_photos": {"$exists": True}}).to_list(length=None)
        
        cleaned_count = 0
        invalid_files_found = []
        
        logger.info(f"Found {len(weddings)} weddings with layout_photos")
